...
import { getBonusList } from '../../../../redux/slices/bonus';
// @types
...
export default function BonusList() {
  ...
  const { loading, bonusList } = useSelector((state: { bonus: bonusState }) => state.bonus);
  const [page, setPageNo] = useState(1);

  useEffect(() => {
    dispatch(getBonusList({ bonusPageNo: page }));
  }, [dispatch, page]);
  ...
  const CustomPagination = () => (
    <Pagination
      color="primary"
      count={Math.ceil(get(bonusList, 'totalRecords') / 10)}
      page={page}
      onChange={(event, value) => {
        setPageNo(value);
        dispatch(getBonusList({ bonusPageNo: value }));
      }}
    />
  );
  ...
  return (
    <>
      <BrandRowWrapper>
        ...
        >
          {'Bonus List'.split('').map((letter, index) => (
      ...
      {!loading && (
        <DataGrid
          autoHeight
          rowHeight={100}
          rows={mapDataToTargetStructure(get(bonusList, 'data', []))}
          columns={columns}
          pagination
          pageSize={10}
          components={{
