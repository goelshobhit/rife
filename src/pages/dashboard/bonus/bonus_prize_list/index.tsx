...
import { getBonusPrizeList } from '../../../../redux/slices/bonus';
// @types
...
export default function BonusPrizeList() {
  const dispatch = useDispatch();
  const { loading, bonusPrizeList } = useSelector((state: { bonus: bonusState }) => state.bonus);
  const [page, setPageNo] = useState(1);

  useEffect(() => {
    dispatch(getBonusPrizeList({ bonusPageNo: page }));
  }, [dispatch, page]);
  ...
  const CustomPagination = () => (
    <Pagination
      color="primary"
      count={Math.ceil(get(bonusPrizeList, 'totalRecords') / 10)}
      page={page}
      onChange={(event, value) => {
        setPageNo(value);
        dispatch(getBonusPrizeList({ bonusPageNo: value }));
      }}
    />
  );
  ...
  return (
    <>
      <BrandRowWrapper>
        ...
        >
          {'Bonus Prize List'.split(' ').map((letter, index) => (
      ...
      {!loading && (
        <DataGrid
          autoHeight
          rowHeight={100}
          rows={mapDataToTargetStructure(get(bonusPrizeList, 'data', []))}
          columns={columns}
          pagination
          pageSize={10}
          components={{
