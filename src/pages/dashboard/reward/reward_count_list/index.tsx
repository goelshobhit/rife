...
import { getRewardCountList } from '../../../../redux/slices/reward';
// @types
...
export default function RewardCountList() {
  const dispatch = useDispatch();
  const { loading, rewardCount } = useSelector((state: { reward: rewardState }) => state.reward);
  const [page, setPageNo] = useState(1);

  useEffect(() => {
    dispatch(getRewardCountList({ bonusPageNo: page }));
  }, [dispatch, page]);
  ...
  const CustomPagination = () => {
    return (
      <Pagination
        color="primary"
        count={Math.ceil(get(rewardCount, 'totalRecords') / 10)}
        page={page}
        onChange={(event, value) => {
          setPageNo(value);
          dispatch(getRewardCountList({ bonusPageNo: value }));
        }}
      />
    );
  };
  ...
  return (
    <>
      <BrandRowWrapper>
        ...
        >
          {'Reward Count List'.split('').map((letter, index) => (
      ...
      {!loading && (
        <DataGrid
          autoHeight
          rowHeight={100}
          rows={mapDataToTargetStructure(get(rewardCount, 'data', []))}
          columns={columns}
          pagination
          pageSize={10}
          components={{
